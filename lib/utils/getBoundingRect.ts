...
export function getBoundingRect(event: MouseEvent | TouchEvent) {
  ...
  const pageX = isTouchEvent(event) ? (event.touches || event.changedTouches)[0].pageX : event.pageX;
  const pageY = isTouchEvent(event) ? (event.touches || event.changedTouches)[0].pageY : event.pageY;
  return {
